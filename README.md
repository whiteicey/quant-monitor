...
- K线图、MACD、RSI 实时更新
- **所有交易信号实时更新**：强买/强卖/买入/卖出/MACD金叉/死叉，盘中信号翻转时自动清除旧标记
- 多空评分、信号状态、关键价位实时重算
- 交易时段/收盘状态自动识别
- **智能网络代理**：自动检测企业代理/家庭直连，兼容各种网络环境

### 技术指标
- **EMA** 快慢线 + 金叉/死叉
- **MACD** 双线 + 柱状图 + **金叉死叉标记（红色上箭头/绿色下箭头）**
- **RSI** 超买超卖
- **布林带** 上中下轨
...
| **过拟合** | 13策略×13预设的"胜率100%"为样本内结果 | v2.1 参数优化+过拟合检测 |
| **统计检验** | 无Sharpe置信区间、无t检验 | v2.1 已实现 |
