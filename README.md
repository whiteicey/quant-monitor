...
- [ ] 信号提醒 — 自选股信号变化时弹窗/声音提醒（接口已预留: `src/extensions.py`）
- [ ] 仓位管理 — 分仓买入、金字塔加仓、Kelly公式（接口已预留）
- [ ] 止盈止损 — 移动止损、固定止损、ATR动态止损（接口已预留）
...
### 策略研究
- [ ] 更多技术指标 — KDJ、OBV、VWAP（接口已预留: `src/extensions.py`）
- [ ] 机器学习 — 用历史数据训练信号分类模型
- [ ] 多周期共振 — 日线+周线信号同时确认时才触发（接口已预留）
- [ ] 板块联动 — 分析个股所属板块整体强弱
